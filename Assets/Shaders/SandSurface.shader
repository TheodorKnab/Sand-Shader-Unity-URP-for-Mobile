...
Shader "Universal Render Pipeline/temp_name/SandShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                //Calculate Normal from 2nd Normal Map, provided in Grayscale     
                half3 graynorm = half3(0, 0, 1);
                half2 dispUV = half2( 1 - input.uv.x,input.uv.y);
                half heightSampleCenter = tex2D (_DispMap, dispUV).r;
                half heightSampleRight = tex2D (_DispMap, dispUV + half2(_DispMap_TexelSize.x, 0)).r;
                half heightSampleUp = tex2D (_DispMap, dispUV + half2(0, _DispMap_TexelSize.y)).r;
                half sampleDeltaRight = heightSampleRight - heightSampleCenter;
                half sampleDeltaUp = heightSampleUp - heightSampleCenter;
                half dispStrength = _DispStrength;
                graynorm = cross(float3(1, 0, sampleDeltaRight * dispStrength), float3(0, 1, sampleDeltaUp * dispStrength));  
                graynorm *= half3(-1,1,1); 
                ...
                //combine both normalMaps
                normalWS = normalize(normalWS + graynorm.xzy);
